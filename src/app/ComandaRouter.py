...
async def create_comanda(comanda_data: ComandaCreate, request: Request, db: AsyncSession = Depends(get_async_db), current_user: FuncionarioAuth = Depends(get_current_active_user)):
  try:
    ...
    await AuditoriaService.registrar_acao(db=db, funcionario_id=current_user.id, acao="CREATE", recurso="COMANDA", recurso_id=new_comanda.id, dados_novos=new_comanda, request=request)
...
Depends(get_current_active_user)):
  try:
    ...
    await AuditoriaService.registrar_acao(db=db, funcionario_id=current_user.id, acao="CREATE", recurso="COMANDA_PRODUTO", recurso_id=new_comanda_produto.id, dados_novos=new_comanda_produto,
    request=request)
